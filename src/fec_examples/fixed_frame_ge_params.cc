...
int main(int argc, char* argv[])
{
    ...
    double t_bg, t_gb, l_g, l_b;
    if (loss_metric.mean_frame_burst_length == 0.0 ||
        loss_metric.mean_frame_guardspace_length == 0.0) {
        t_gb = 0.0;
        t_bg = 0.0;
        l_b = loss_metric.loss_fraction;
        l_g = loss_metric.loss_fraction;
    } else {
        t_bg = 1.0 / loss_metric.mean_frame_burst_length;
        t_gb = 1.0 / loss_metric.mean_frame_guardspace_length;
        l_b = loss_metric_computer.compute_multi_frame_loss_fraction(loss_info.packet_losses,
                loss_info.indices, 2);
        l_g = loss_metric.loss_fraction * (t_bg + t_gb) / t_bg 
            - l_b * t_gb / t_bg;
    ...
    std::cout << t_gb << " " << t_bg << " " << l_g << " " << l_b << std::endl;
