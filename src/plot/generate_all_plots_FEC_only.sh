...
usage() {
    echo "generate_all_plots_FEC_only.sh --config= --plot-folder="
    exit 1
}
...
python3 $SCRIPT_DIR/cdf_non_recoverable_frames.py --log $LOG --config $CONFIG --output $PLOT/cdf_recoverable_frames.svg --skip StreamingCode_3-point_9 --no_video true
...
python3 $SCRIPT_DIR/burst_losses.py --log $LOG --config $CONFIG --output $PLOT/mean_recovered_frames_bursts.svg --mean --max_burst 4 --offset -4 --skip StreamingCode_3-point_9