// Modified from https://github.com/microsoft/ringmaster/blob/main/src/app/protocol.cc
#include <stdexcept>
#include "protocol.hh"
...
shared_ptr<Msg> Msg::parse_from_string(const string & binary)
{
  ...
  if (type == Type::ACK) {
    ...
    auto ret = make_shared<FECMsg>();
    ret->info = parser.read_uint8();
    return ret;
  }
  else if (type == Type::FRAME) {
    auto ret = make_shared<FRAMEMsg>();
    ret->frame_id = parser.read_uint32();
    return ret;
  }
  else {
...
FECMsg::FECMsg(const FeedbackDatagram & datagram) : Msg(Type::FEC),
    info(datagram.qr) {}

size_t FECMsg::serialized_size() const
{
  return Msg::serialized_size() + sizeof(uint8_t);
}

string FECMsg::serialize_to_string() const
{
  string binary;
  binary.reserve(serialized_size());

  binary += Msg::serialize_to_string();
  binary += put_number(info);

  return binary;
}

FRAMEMsg::FRAMEMsg(uint32_t frame_id_val) : Msg(Type::FRAME),
    frame_id(frame_id_val) {}

size_t FRAMEMsg::serialized_size() const
{
  return Msg::serialized_size() + sizeof(uint32_t);
}

string FRAMEMsg::serialize_to_string() const
{
  string binary;
  binary.reserve(serialized_size());
