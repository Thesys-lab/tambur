// Modified from https://github.com/microsoft/ringmaster/blob/main/src/app/protocol.hh
#ifndef PROTOCOL_HH
#define PROTOCOL_HH
...
enum class FrameType : uint8_t {
  ...
  DUMMY = 3, // temporary value for a dummy frame
};
...
struct Msg
{
  enum class Type : uint8_t {
    ...
    CONFIG = 2,   // ConfigMsg
    FEC = 3, //FECMsg
    FRAME = 4 // Ack that a frame was decoded
  };
...
struct FECMsg : Msg
{
  // construct an FECMsg
  FECMsg() : Msg(Type::FEC) {}
  FECMsg(const FeedbackDatagram & datagram);

  uint8_t info {}; // information of feedback
...
struct FRAMEMsg : Msg
{
  // construct an FrameMsg
  FRAMEMsg() : Msg(Type::FRAME) {}
  FRAMEMsg(uint32_t frame_id);
